...
export const Calendar = withStyles(styles)(
  class extends React.Component {
    ...
    render() {
      ...
      const { columns, itemSize, itemsDistance } = this.props;
      const rects = [];
      ...
      for (let i = 0; i < columns; ++i) {
        const lastDayOfWeek = i === columns - 1 ? this.props.today.day() : 6;
        const currentWeekStarts = moment(fdow).day(-7 * (columns - 1 - i));
        for (let j = 0; j <= lastDayOfWeek; ++j) {
          const date = moment(currentWeekStarts).day(j);
          const color = this.props.itemColor && this.props.itemColor(date);
          rects.push(
            <rect
              className={rect}
              key={`${i}-${j}-fill`}
